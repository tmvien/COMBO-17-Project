...
for(m in 1:iterations){
  ...
  cv.lasso  <-  cv.glmnet(X[train,], y[train], alpha = 1, family = "gaussian", intercept = T)
  ...
  lasso.fit = glmnet(X[train,], y[train], alpha = 1, family = "gaussian",
                     intercept = T, lambda = cv.lasso$lambda.min,
                     standardize = F)
  end.time  <-  proc.time() - start.time
  ...
  train.r2[m,1] = 1 - (mean((y[train] - train.pred)^2)/mean((y-mean(y))^2))
  
  test.pred = as.vector(predict(lasso.fit, newx = X[-train,], type="response"))
  lasso.test.resid = y[-train] - test.pred
  test.r2[m,1] = 1 - (mean((y[-train] - test.pred)^2)/mean((y-mean(y))^2))

  ############# elastic net
  # begin cross-validation
  start.time  <- proc.time()
  cv.el = cv.glmnet(X[train,], y[train], alpha = 0.5, family = "gaussian", intercept = T)
  end.time  <-  proc.time() - start.time
  time.cv[m,2]  = end.time["elapsed"]
  cv.error[m,2] = min(cv.el$cvm)
  # begin model fit
  start.time  <- proc.time()
  el.fit = glmnet(X[train,], y[train], alpha = 0.5, family = "gaussian",
                  intercept = T, lambda = cv.el$lambda.min,
                  standardize = F)
  end.time  <-  proc.time() - start.time
  ...
  train.r2[m,2] = 1 - (mean((y[train] - train.pred)^2)/mean((y-mean(y))^2))

  test.pred = as.vector(predict(el.fit, newx = X[-train,], type="response"))
  el.test.resid = y[-train] - test.pred
  test.r2[m,2] = 1 - (mean((y[-train] - test.pred)^2)/mean((y-mean(y))^2))
    
  ############## ridge 
  # begin cross-validation
  start.time  <- proc.time()
  cv.rid = cv.glmnet(X[train,], y[train], alpha = 0, family = "gaussian", intercept = T)
  end.time  <-  proc.time() - start.time
  time.cv[m,3]  = end.time["elapsed"]
  cv.error[m,3] = min(cv.rid$cvm)
  # begin model fit
  start.time  <- proc.time()
  rid.fit = glmnet(X[train,], y[train], alpha = 0, family = "gaussian", 
  ...
  train.r2[m,3] = 1 - (mean((y[train] - train.pred)^2)/mean((y-mean(y))^2))

  test.pred = as.vector(predict(rid.fit, newx = X[-train,], type = "response"))
  rid.test.resid = y[-train] - test.pred
  test.r2[m,3] = 1 - (mean((y[-train] - test.pred)^2)/mean((y-mean(y))^2))
  ...
  train.r2[m,4] = 1 - (mean(y[train] - train.pred)^2)/mean((y-mean(y))^2)
    
  test.pred = predict(rf, newdata = X[-train,])
  rf.test.resid = y[-train] - test.pred
  test.r2[m,4] = 1 - (mean(y[-train] - test.pred)^2)/mean((y-mean(y))^2)
...
#         train.r2[m,i]  <-  1 - (mean((y[train] - train.pred)^2)/mean((y-mean(y))^2))

#         test.pred      <-  as.vector(predict(mod, newx = X[-train,], type="response"))
#         #test.resid    <-  y[-train] - test.pred
#         test.r2[m,i]   <-  1 - (mean((y[-train] - test.pred)^2)/mean((y-mean(y))^2))
#         if (m == 100) {
...
#     train.r2[m,4]   <-  1 - (mean(y[train] - rf.pred.train)^2)/mean((y-mean(y))^2)
#     rf.pred.test    <-  predict(rf, newdata = X[-train,])
#     rf.test.resid   <-  y[-train] - rf.pred.test
#     test.r2[m,4]    <-  1 - (mean(y[-train] - rf.pred.test)^2)/mean((y-mean(y))^2)
# }
...
#plot(c17$UbMAG, c17$Mcz, xlab="Bersell U luminosity", ylab="Redshift Magnitude")
...
ggplot(c17, aes(x = UbMAG, y = Mcz)) +
  ...
  ggtitle("Redshift vs Bershell U Luminosity")
